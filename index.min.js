class b extends HTMLElement{static get observedAttributes(){return["network","handle","href"]}attributeChangedCallback(a,c,d){if(!this.__initialized)return;c!==d&&(this[a]=d)}get network(){return this.getAttribute("network")}set network(a){this.setAttribute("network",a),this.setNetwork()}get handle(){return this.getAttribute("handle")}set handle(a){this.setAttribute("handle",a),this.setHandle()}get href(){return this.getAttribute("href")}set href(a){this.setAttribute("href",a),this.setHref()}constructor(){super();this.__initialized=!1,this.attachShadow({mode:"open"}),this.__svgs=document.createElementNS("http://www.w3.org/2000/svg","svg"),this.__svgs.style.display="none",this.__svgs.innerHTML=b.svgs(),this.shadowRoot.appendChild(this.__svgs),this.__template=document.createElement("template"),this.__template.innerHTML=b.template(),this.shadowRoot.appendChild(this.__template.content.cloneNode(!0)),this.__networks=b.networks(),this.__use=this.shadowRoot.querySelector("use")}async connectedCallback(){this.setAttribute("role","link"),this.hasAttribute("network")&&this.setNetwork(),this.hasAttribute("handle")&&this.setHandle(),this.hasAttribute("href")&&this.setHref(),this.__initialized=!0}setNetwork(){const a=this.getAttribute("network");this.__use.setAttribute("href",`#${a}`);const c=`${this.__networks[a].label} Link`;this.setAttribute("aria-label",c)}setHandle(){const a=this.getAttribute("handle"),c=this.getAttribute("network");if(!this.hasAttribute("href")){const d=`${this.__networks[c].href}${a}`;this.shadowRoot.querySelector("a").href=d}}setHref(){const a=this.getAttribute("href");this.shadowRoot.querySelector("a").href=a}static template(){return`
      <style>
        :host {
    ...
    </svg>`}}customElements.define("wc-social-link",b);export{b as WCSocialLink};
