var e=class extends HTMLElement{static get observedAttributes(){return["network","handle","href"]}attributeChangedCallback(t,i,s){!this.__initialized||i!==s&&(this[t]=s)}get network(){return this.getAttribute("network")}set network(t){this.setAttribute("network",t),this.setNetwork()}get handle(){return this.getAttribute("handle")}set handle(t){this.setAttribute("handle",t),this.setHandle()}get href(){return this.getAttribute("href")}set href(t){this.setAttribute("href",t),this.setHref()}constructor(){super();this.__initialized=!1,this.attachShadow({mode:"open"}),this.__svgs=document.createElementNS("http://www.w3.org/2000/svg","svg"),this.__svgs.style.display="none",this.__svgs.innerHTML=e.svgs(),this.shadowRoot.appendChild(this.__svgs),this.__template=document.createElement("template"),this.__template.innerHTML=e.template(),this.shadowRoot.appendChild(this.__template.content.cloneNode(!0)),this.__networks=e.networks(),this.__use=this.shadowRoot.querySelector("use")}async connectedCallback(){this.setAttribute("role","link"),this.hasAttribute("network")&&this.setNetwork(),this.hasAttribute("handle")&&this.setHandle(),this.hasAttribute("href")&&this.setHref(),this.__initialized=!0}setNetwork(){let t=this.getAttribute("network");this.__use.setAttribute("href",`#${t}`);let i=`${this.__networks[t].label} Link`;this.setAttribute("aria-label",i)}setHandle(){let t=this.getAttribute("handle"),i=this.getAttribute("network");if(!this.hasAttribute("href")){let s=`${this.__networks[i].href}${t}`;this.shadowRoot.querySelector("a").href=s}}setHref(){let t=this.getAttribute("href");this.shadowRoot.querySelector("a").href=t}static template(){return`
      <style>
        :host {
    ...
    </svg>`}};customElements.define("wc-social-link",e);export{e as WCSocialLink};
